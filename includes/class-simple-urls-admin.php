<?php
/**
 * Really_Simple_Urls_Admin file.
 *
 * @package really-simple-urls
 */

/**
 * Really_Simple_Urls_Admin class.
 */
class Really_Simple_Urls_Admin {
	...
	public function columns_filter( $columns ) {
		...
		$columns = array(
			...
			'title'     => __( 'Title', 'really-simple-urls' ),
			'url'       => __( 'Redirect to', 'really-simple-urls' ),
			'permalink' => __( 'Permalink', 'really-simple-urls' ),
			'clicks'    => __( 'Clicks', 'really-simple-urls' ),
		);
	...
	public function add_meta_box() {
		add_meta_box( 'surl', __( 'URL Information', 'really-simple-urls' ), array( $this, 'meta_box' ), 'surl', 'normal', 'high' );
	}
	...
	public function meta_box() {
		...
		printf( '<p><label for="%s">%s</label></p>', '_surl_redirect', esc_html__( 'Redirect URI', 'really-simple-urls' ) );
		...
		printf( '<p><span class="description">%s</span></p>', esc_html__( 'This is the URL that the Redirect Link you create on this page will redirect to when accessed in a web browser.', 'really-simple-urls' ) );

		$count = isset( $post->ID ) ? get_post_meta( $post->ID, '_surl_count', true ) : 0;
		/* translators: %d is the counter of clicks. */
		echo '<p>' . sprintf( esc_html__( 'This URL has been accessed %d times', 'really-simple-urls' ), esc_attr( $count ) ) . '</p>';
